      ...
      <nav className="max-w-6xl mx-auto rounded-3xl sm:rounded-full border border-white/10 bg-neutral-950/85 backdrop-blur-xl shadow-2xl shadow-black/60 transition-all duration-300">
        <div className="px-4 sm:px-6 h-16 flex items-center justify-between">
          ...
          <Link href="/" className="flex items-center gap-3 group">
            <div className="relative w-9 h-9 bg-gradient-to-tr from-indigo-600 via-indigo-500 to-purple-600 rounded-xl flex items-center justify-center shadow-lg shadow-indigo-500/25 group-hover:scale-105 transition-transform">
            ...
            <div className="flex flex-col">
              <span className="text-base sm:text-lg font-black tracking-tight text-white group-hover:text-indigo-300 transition-colors">
                SpaceCraft <span className="text-xs font-mono font-bold text-indigo-400 uppercase tracking-widest">3D</span>
              </span>
              <span className="hidden sm:inline text-[9px] font-mono text-neutral-400 -mt-1">Cloud Arch Platform</span>
            </div>
          </Link>
          ...
          <div className="flex items-center gap-2 sm:gap-4 flex-shrink-0">
            {session ? (
              <Link
                href="/dashboard"
                className="hidden sm:inline text-xs font-bold uppercase tracking-wider text-neutral-300 hover:text-white transition-colors whitespace-nowrap"
              >
                Dashboard
              </Link>
            ) : (
              <button
                onClick={() => signIn()}
                className="hidden sm:inline text-xs font-bold uppercase tracking-wider text-neutral-300 hover:text-white transition-colors whitespace-nowrap px-3 py-1.5 rounded-lg hover:bg-white/5"
              >
                Giriş Et
              </button>
            )}
            ...
            >
              ...
              <span className="relative px-5 py-2.5 rounded-full bg-neutral-950 text-white flex items-center gap-2 group-hover:bg-neutral-900 transition-all">
            ...
            <button
              ...
              className="md:hidden p-2 text-neutral-300 hover:text-white rounded-xl hover:bg-white/10 transition-colors"
              title="Menyunu Aç"
            >
        ...
        {isMobileMenuOpen && (
          <div className="md:hidden border-t border-white/10 px-6 py-4 flex flex-col gap-3 text-xs font-semibold uppercase tracking-wider text-neutral-300 animate-fadeIn bg-neutral-950/95 rounded-b-3xl">
            {navLinks.map((link) => {
              const isActive = pathname === link.href;
              return (
                <Link
                  ...
                  className={`py-1.5 ${isActive ? "text-indigo-400 font-bold" : "hover:text-white"}`}
                >
                  {link.label}
                </Link>
              );
